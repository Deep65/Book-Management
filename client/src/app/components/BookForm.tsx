...
import { ADD_BOOK, UPDATE_BOOK } from "../../../lib/mutations/book";
...
const BookForm: React.FC<BookFormProps> = ({ bookId, mode }) => {
  ...
  const [addBook, { loading: addLoading, error: addError }] = useMutation(
    ADD_BOOK,
    {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {mode === "add" && addError && (
            <p className="text-red-500 text-sm mt-4">{addError.message}</p>
          )}
          {mode === "update" && updateError && (
            <p className="text-red-500 text-sm mt-4">{updateError.message}</p>
          )}
        </form>
      </div>
    </div>
